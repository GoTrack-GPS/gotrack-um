...
<table>
  <tr>
        <td colspan="3">* Updated after OK feedback</td>
    </tr> 
    <tr>
...
<table>
    <td colspan="3"> * Default is on <br>
                    * Updated after feedback OK (for turning off) 
        </td>
    <tr>
        <td rowspan="2">Structure  </td>
        <td>Write </td>
...
<table>
     <tr>
        <td colspan="3"> * Updated after feedback OK
        </td>
    </tr> 
    <tr>
        <td rowspan="2">Structure  </td>
...
##### 1.14. Record driving information on an RFID card

- It is necessary to place the driver card that needs information on the surface of the device (Gotrack logo position) before sending the recording command.
- 1 BIP sound will sound to signal that the card is successful, now you can take out the card.

<table>
    <tr>
        <td colspan="3"> * To check the information written on the card: swipe the card into the device to perform the driver login. Then send the command read: <br> view,driverinfo#
        </td>
    </tr>
    <tr>
        <td rowspan="2">Structure</td>
    ...
    <tr>
     ...
     <tr>
        <td>Submit </td>
        <td>Driver's name: Nguyen Van A and gplx number: gplx123456
...
Notice BIP if successful recording
</td>
        <td>
    </tr>
...
##### 1.15. Setting the power consumption balance (low voltage)
...
### 4. Configure RS232 ports
- Figure 9 lists the sensors that the device supports connecting via RS232 port.
- **Note**:
     - **S400E has only 1 RS232 port, so parameter X will only take a value of 0 for all configurations of rs232 port**.
